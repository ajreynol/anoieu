...
| **M2** ✅ | desugaring + `desugar`/`symbol` commands, validated against ethos | the surface↔core map, and the conformance harness: 33 cases, one per policy, agreeing with ethos term for term |
...
| **M4** ◐ | `.eos` front end + triple checks, baselines, JSON/SARIF | baselines, suppression comments, configuration files, SARIF and multiple entry points are written -- see [`ci.md`](ci.md). What remains is the `.eos` front end itself: the `is-list-nil` diff, exclusion closure, the coverage matrix |
