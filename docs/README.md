...
| [`bugs.json`](reports/bugs.json) | **the database, and the raw data** — every bug the checks have ever found, one JSON object each, readable in a browser with no clone. `scripts/run_static_analysis` dumps a run and [koine](https://github.com/ajreynol/koine)'s `koine_append_db` adds what is new to this. *Append-only: nothing in it is edited or removed* |
| [`static-analysis.md`](reports/static-analysis.md) | **the table rendered from it**, the new workflow's page. Not the report: `open-findings.md` is still that, and nothing here closes a bug. *Rewritten whole, from the database* |
