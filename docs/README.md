...
| [`bugs.json`](reports/bugs.json) | **the database, and the raw data** — every bug the checks have ever found, one JSON object each, readable in a browser with no clone. `scripts/anoieu_analyzer` dumps a run and [koine](https://github.com/ajreynol/koine)'s `koine_append_db` adds what is new to this. *Append-only: nothing in it is edited or removed* |
