...
| --- | --- | --- |
| **discussion** | a topic in [`discussion.md`](discussion.md) addressed to them, or a reply into a topic of theirs — the standing channel for anything that is **not** a defect report | a person carries it; `scripts/process_discussion` reads theirs, and nothing here writes into anybody else's file |
| **findings** | a row in the ledger, carried by `scripts/check_anoieu <id>` and answered through `scripts/process_anoieu` | the same person, through the reporting workflow, which is a separate protocol on purpose |
...
**Channel:** **findings** — `ethos-8` and `ethos-9`, through `scripts/check_anoieu ethos`, because a defect report is not a discussion topic.
...
**Prompt — `anoieu`:** run `scripts/check_anoieu ethos` for these two rows, and
...
**Channel:** **findings** — `cvc5-1` in the ledger, carried by `scripts/check_anoieu cvc5`. `cvc5` has no discussion file and has joined nothing; delivery upstream is a person's act.
