...
counts as a bug of whichever one changed or ruled to close it, never both. A
single episode can close several bugs, so the tally and the episode count
measure different things. It is regenerated from the
database by `python3 -m anoieu_analyzer.reporting.database`, and `--check`
fails when it is stale, so no closure has to remember to update it.

## Running tally

<!-- tally:begin (generated by anoieu_analyzer.reporting.database) -->
...
## E7: ethos fixed a crash and a duplicate-macro disagreement the fuzzer found, and both rows now wait on a replay

| | |
| --- | --- |
| **When** | 2026-09-23 |
| **Kind** | positive. The pull request is titled *Address more issues from anoieu*, and its body says the issues were *"Found in latest round of fuzz testing ethos vs logos"* |
| **Ours** | `7836f9530ce4994c` (`FUZ0002`) and `4a419fe0339d88b6` (`FUZ0001`), both from the differential fuzzer |
| **Theirs** | ethos [#245](https://github.com/cvc5/ethos/pull/245), [`08785469`](https://github.com/cvc5/ethos/commit/087854695143b3cb085415f1c642b8763419668e) |
| **Outcome** | nothing closed yet. Both rows are open as **candidates for replay** against an ethos built at or after `08785469`. They are claims about what the binary does, and reading a diff cannot close them |

**What happened.** Our fuzzer runs the same proof file through ethos and logos,
two independent checkers for the Eunoia language, and records every file on
which they disagree or one of them crashes. ethos answered two of the findings
recorded on 2026-09-22 the next day, in
[`08785469`](https://github.com/cvc5/ethos/commit/087854695143b3cb085415f1c642b8763419668e):

- **An empty application no longer crashes it.** The term `(_)` used to
  segfault ethos with no output at all. `src/expr_parser.cpp` now refuses it
  with *"Expected an operator for _"*. The new failing test
  `tests/apply-no-operator.eo` is our reproducer, `(declare-const y (_))`,
  word for word.
- **A `define` declared twice now behaves like one declared once.** Declaring
  the same macro a second time made it an overload, and ethos did not
  beta-reduce an application of an overloaded macro. So two uses of it
  resolved to different terms, and `contra` then failed where logos accepted.
  `State::getOverloadInternal` in `src/state.cpp` now beta-reduces an
  overloaded macro through the new `mkBetaReduceInternal`, just as `mkExpr`
  does for a macro that is not overloaded.
  `tests/overload-define-lambda.eo` pins the fix.

We confirmed both fixes in the current source rather than in the commit message,
on ethos `main` at `087854695143`: `src/expr_parser.cpp` line 285 and
`src/state.cpp` lines 1876–1879.

**What we learned.**

- **A reduced reproducer is what a maintainer can use directly.** ethos made our
  two-line crash case its regression test unchanged.
- **Filing a disagreement against both checkers was the right call.** The
  duplicate `define` needed a real decision about which checker was right, and
  ethos settled it by moving toward logos. We filed it against `ethos+logos`
  without guessing which side was wrong, and the side that moved was not the
  one a guess would most likely have named.

## E8: logos made its parser refuse literals as declared names and read two forms ethos accepts, and two rows now wait on a replay

| | |
| --- | --- |
| **When** | 2026-09-23 |
| **Kind** | positive. The pull request is titled *Address more issues from anoieu* |
| **Ours** | `1cc815407beb3759` (`FUZ0001`) and `26af98b965618686` (`FUZ0005`), both from the differential fuzzer |
| **Theirs** | logos [#471](https://github.com/cvc5/logos/pull/471), [`2afc6986`](https://github.com/cvc5/logos/commit/2afc6986c91dd60f1fecfbec7d35d300671db8ff); it relies on ethos [#246](https://github.com/cvc5/ethos/pull/246), [`0b7e60d6`](https://github.com/cvc5/ethos/commit/0b7e60d65da2d1911bdf0ae3d8ce230355f9785f), whose own body calls it a prerequisite for this change |
| **Outcome** | nothing closed yet. Both rows are open as **candidates for replay** against a logos built at or after `2afc6986`. Four more rows from the same campaign stay open (see *Not closed*) |

**What happened.** Our fuzzer runs the same proof file through ethos and logos,
two independent checkers for the Eunoia language, and records every file on
which they disagree. logos answered the findings recorded on 2026-09-22 the
next day by changing its parser, `Logos/Parser.lean`, in
[`2afc6986`](https://github.com/cvc5/logos/commit/2afc6986c91dd60f1fecfbec7d35d300671db8ff):

- **Every declared name must now be a symbol.** Datatype, constructor and
  selector names, `define` names, macro parameters and `let` bindings all go
  through `parseSymbol`, which refuses a literal, a keyword or a string. logos
  used to accept `#b1` as a nullary constructor, which silently changed what
  that literal meant for the rest of the proof. ethos refuses it.
- **It reads the singular `declare-datatype`** as a one-datatype
  `declare-datatypes` block.
- **It reads `forall` and `exists` binder syntax.** This goes through a new
  `binder` field on operator declarations and needs the matching `:binder`
  support in ethos's compiler, which ethos added in
  [`0b7e60d6`](https://github.com/cvc5/ethos/commit/0b7e60d65da2d1911bdf0ae3d8ce230355f9785f).

We confirmed the fixes in the current source rather than in the commit message,
on logos `main` at `49f4fd1f256f`: `Logos/Parser.lean` line 423
(`parseSymbol`), lines 754–764 (datatype, constructor and selector names), and
line 994 (`declare-datatype`).

**Not closed.**

- **`0e9250` (`FUZ0005`, a `lambda` in an assumption) is still open.** Our note
  on it said `forall` and `exists` fail on the same path, and logos fixed those
  two. But the reproducer is a `lambda`, and logos's `docs/parser.md` at
  `49f4fd1f` now says that *"`lambda` is a binder in CPC too, but Logos has no
  representation for it, so a term using it is not read"*.
- **`147d8c` (`FUZ0005`, a parametric datatype) is still open.**
  `Logos/Parser.lean` lines 756 and 848 still throw *"parametric datatype … is
  not supported"*. The new `test/CpcParser.lean` guards also pin the refusal for
  the singular form.
- **`acb5d4` (`FUZ0005`, `(<= 7)`) and `1e92c8` (`FUZ0001`, a step whose stated
  conclusion is not what `contra` derives) are still open.** No commit in the
  logos or ethos window touches them.

**What we learned.**

- **A bucket's note can promise more than its reproducer tests.** The `lambda`
  bucket's note named `forall` and `exists` as siblings. logos fixed exactly
  those two and wrote down why it will not read the third. A replay of the
  bucket will report no change, and nothing on the row will show that two
  thirds of what its note described is fixed.
- **Some of these rows may be heading for a ruling rather than a fix.** logos
  has now documented its refusals of `lambda` and of parametric datatypes. If
...
A maintainer reads the diff. No entry is written by hand. **An entry does not
wait for a closure:** a change that answers something of ours earns its entry in
the run that reads it, even when none of its rows can close yet. The usual case
is a fix for a fuzzer row, which needs a replay. That entry says in `Outcome`
what is still owed, and the pass that later closes the rows writes a new entry
naming it by id rather than rewriting it. The [running tally](#running-tally) is
the one part of this page that is generated, and nobody edits it.
...
| an observation nobody has ruled on, and nobody there has acted on | nothing has happened to it | the database, as `open`, and the tally |
