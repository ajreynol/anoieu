...
(`NOT_AUDITED` in `scripts/gen_corpus_table.py`), and whether the copy has drifted
...
| [`closed-findings.md`](closed-findings.md) | every finding ruled on, with its verdict; `scripts/landing.py` audits the ones closed before their fix landed |
...
python3 scripts/sweep.py <ethos>/tests <logos>/install/defs <eudaimonia>/examples
...
| **accepted, closed, awaiting landing** | 7 | ethos-1, ethos-7, ethos-8, ethos-9 and the `symm` docstring — seven rows on `anoieu-findings`@`292201c2`, tracked by `scripts/landing.py` |
...
- **files a project did not author are not audited** (`NOT_AUDITED` in
  `scripts/gen_corpus_table.py`), after seventeen rows were filed against logos
...
- **a merge is no longer what closes a row** — a maintainer's acceptance and a
  ...
  [`scripts/landing.py`](../../scripts/landing.py) so that the shortcut is booked rather
...
292201c2`, and `python3 scripts/landing.py --check` is the pass that asks whether
