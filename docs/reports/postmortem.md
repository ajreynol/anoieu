...
what [`prompts/process_anoieu`](../../prompts/process_anoieu) runs. Not the project
...
than the entry.
    ...
    prompts/process_anoieu <project> [ID]              # an entry is written
    prompts/process_anoieu --no-postm <project> [ID]   # the agent decides
...
passed to `prompts/process_anoieu`, so it is never a judgement call.
...
`anoieu_analyzer/reporting/landing.py`, asked of the commit rather than of the person. The loop got
...
[`anoieu_analyzer/reporting/landing.py`](../../anoieu_analyzer/reporting/landing.py) reads those back and asks each
