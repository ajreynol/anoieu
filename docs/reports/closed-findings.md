...
1. **It is what makes a verdict stick.** `anoieu_analyzer/reporting/gen_open_findings.py` skips any
...
and `python3 -m anoieu_analyzer.reporting.landing --check` reads every one of them back and asks the
...
| `eac7ccd4d5fb0953` | logos | TRI0002 | `install/defs/Cpc.eos:542` | the semantics has an entry for `str.indexof_re_split`, which the signature does not declare | fixed and landed — the entry is gone from `install/defs/Cpc.eos` on logos `main`, checked at `6cb59db5` (2026-08-31), and the run of 2026-08-31 reports no `TRI0002` on the triple. It had been held open against `updateCompiler`, a branch that has since been deleted; logos is now measured on `main`, which is what the finding was always about |
