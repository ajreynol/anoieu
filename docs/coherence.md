...
| `deploy <president> --version=X.Y.Z` | here | **the first fragment of the epoch build system**, doing by program what `epoch deploy` describes: check the gates, close the stretch, open the next, and leave the incoming president a `FIXME` in the record. Refuses if the president is not a member, if the stretch is not `staged`, if our policy check fails, or if the bump gate is not green. `--dry-run` evaluates everything and writes nothing. **No `--force`**. It notes when the version asked for disagrees with the one the stretch log already intends — `E1` intends `0.1.0` |
