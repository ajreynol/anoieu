...
`bug_db_manager/koine_append_db`. `report` records the promoted corpus through the same
...
`--corpus`. There is no local database fallback. `anoieu_analyzer/reporting/koine.py` locates the
required tool through `$KOINE`, `../koine`, or a clone at `anoieu_analyzer/reporting/config/koine.lock`;
...
> **None of it is confirmed against the commits `anoieu_analyzer/reporting/config/deps.lock` records.** It
