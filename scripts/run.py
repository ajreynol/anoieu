...
2. **Measure.** `bug_db/corpus.md`, rewritten whole: the commits that were read,
...
CORPUS = os.path.join(ROOT, "bug_db", "corpus.md")

sys.path.insert(0, ROOT)

from anoieu_analyzer.reporting import deps as deps_mod  # noqa: E402
from anoieu_analyzer.reporting import gen_corpus_table as corpus_mod  # noqa: E402

from anoieu_analyzer import __version__  # noqa: E402
...
def render_corpus(synced: list, rows: list) -> str:
    """`bug_db/corpus.md`: the versions a run read, and what the checks said about
...
def main() -> int:
    ...
    ap.add_argument(
        "--pinned",
        action="store_true",
        help="re-measure the commits bug_db/corpus.md records, not each tip",
    )
    args = ap.parse_args()
    ...
    written = [
        (CORPUS, "bug_db/corpus.md", render_corpus(synced, rows)),
    ...
    if astray and args.check:
        print(
            ...
            "anoieu_analyzer/reporting/config/deps.lock records, so nothing below was measured against what "
            "the report claims. Fix the pin or the ref in anoieu_analyzer/reporting/config/deps.json; "
